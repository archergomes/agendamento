...
<body class="bg-gray-50">
    <div class="page-wrapper">
        ...
        <div id="sidebar-menu" class="sidebar bg-white shadow-lg">
            ...
            <nav class="sidebar-nav">
                <div class="main-menu">
                    ...
                    <a href="/secretario/pacientes" class="block text-gray-700 active">
        ...
        <main class="main-content">
            <div class="container mx-auto px-4 py-8">
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-2xl font-semibold text-gray-800 mb-6">Gerenciar Pacientes</h2>
                    <!-- Search Bar -->
                    <div class="mb-4">
                        <div class="relative">
                            <input type="text" id="search-input" class="w-full p-2 pl-10 border rounded-lg" placeholder="Pesquisar por nome ou BI..." aria-label="Pesquisar pacientes">
        ...
        <div id="edit-patient-modal">
            <div class="modal-content">
                ...
                <div class="mb-4">
                    <label for="edit-email" class="block text-sm font-medium text-gray-700">Email (opcional)</label>
                    <input type="email" id="edit-email" class="w-full p-2 border rounded">
                </div>
                <div class="flex space-x-2">
                    <button id="save-patient-btn" class="bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition flex-1">
                        Salvar
                    </button>
                    <button id="cancel-patient-btn" class="bg-gray-300 text-gray-700 py-2 px-4 rounded-lg hover:bg-gray-400 transition flex-1">
                        Cancelar
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function renderPatients(searchQuery = '') {
            ...
            let patients = Object.values(userProfiles);
            ...
            if (searchQuery) {
                ...
                patients = patients.filter(patient =>
                    ...
                    patient.bi.toLowerCase().includes(searchQuery)
                );
            }

            if (patients.length === 0) {
                list.innerHTML = '<tr><td colspan="6" class="py-2 px-4 text-gray-600 text-center">Nenhum paciente encontrado.</td></tr>';
                showNotification('Nenhum paciente corresponde à pesquisa.', 'info');
                return;
            }

            list.innerHTML = patients.map((patient, index) => {
                const appointmentCount = bookedAppointments.filter(appt => appt.bi === patient.bi).length;
                return `
                    <tr class="border-t">
                        <td class="py-2 px-4">${patient.name}</td>
                        <td class="py-2 px-4">${patient.phone}</td>
                        <td class="py-2 px-4">${patient.bi}</td>
                        <td class="py-2 px-4">${patient.email || '-'}</td>
                        <td class="py-2 px-4">${appointmentCount}</td>
                        <td class="py-2 px-4">
                            <button class="edit-btn bg-blue-600 text-white py-1 px-2 rounded-lg hover:bg-blue-700" data-bi="${patient.bi}">
                                Editar
                            </button>
                        </td>
                    </tr>
        ...
        function showEditPatientModal(bi) {
            ...
            if (!modal || !nameInput || !phoneInput || !biInput || !emailInput) {
                console.error('Edit patient modal elements not found:', { modal, nameInput, phoneInput, biInput, emailInput });
                return;
            }
            const patient = userProfiles[bi];
            if (!patient) {
            ...
            const saveHandler = () => {
                ...
                userProfiles[bi] = { name, phone, bi, email };
                localStorage.setItem('userProfiles', JSON.stringify(userProfiles));
                renderPatients(document.getElementById('search-input')?.value || '');
                modal.classList.remove('show');
                showNotification('Perfil do paciente atualizado com sucesso!', 'success');
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.addEventListener('click', (e) => {
                ...
                const isModalOpen = document.querySelectorAll('#edit-patient-modal.show').length > 0;
