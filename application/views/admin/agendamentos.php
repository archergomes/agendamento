...
<body class="bg-gray-50">
    ...
    <script>
        ...
        async function fetchAppointments(query = '') {
            try {
                const response = await fetch(`<?php echo site_url('api/get_appointments'); ?>${query ? `?query=${encodeURIComponent(query)}` : ''}`, {
                    method: 'GET',
                    headers: {
        ...
        async function renderTable(query = '') {
            ...
            appointments.forEach(appointment => {
                ...
                row.innerHTML = `
                    <td class="px-4 py-2 border">${appointment.patient_name || 'Desconhecido'}</td>
                    <td class="px-4 py-2 border">${appointment.doctor_name || 'Desconhecido'}</td>
                    ...
                    <td class="px-4 py-2 border">
                        <a href="<?php echo site_url('admin/cad_agendamentos'); ?>?id=${appointment.id}" class="action-btn edit-btn mr-2" title="Editar">
                            <i class="fas fa-edit"></i>
                        </a>
                        <button class="action-btn delete-btn" data-id="${appointment.id}" title="Excluir">
            ...
            document.querySelectorAll('.delete-btn').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    if (confirm(`Tem certeza que deseja excluir o agendamento ${id}?`)) {
                        try {
                            const response = await fetch('<?php echo site_url('api/delete_appointment'); ?>', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                            ...
                            if (result.error) {
                                ...
                                renderTable(query); // Refresh table
                            }
                        } catch (error) {
